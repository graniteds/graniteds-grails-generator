...
<%
    ...
    if (jClass.constraints) {%>
    
    	public static const meta_constraints:Array = [<%    	
    	for (me in jClass.constraints.entrySet()) {
			if (firstConstraint) firstConstraint = false; else { %>, <% }%>
    		{ property: "${me.key}"<% if (me.value.isEmpty()) {%> }<%} else { %>,
				<%
			boolean firstCons = true;
	    	for (me2 in me.value) {
				if (firstCons) firstCons = false; else { %>, <% }%>${me2.key}: ${me2.value}<%
    		}%>
    		}<% }
    	}%>
		]<%
